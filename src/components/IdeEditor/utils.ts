...
export const resolveConfig = (config: any) => {
  // 在链接后添加DEPS
  const addDepsToUrl = (key: string, url: string) => {
    if (['react', 'react-dom'].includes(key)) {
      return url;
    }
    const depsString = Object.entries(config.DEPS)
      .filter(([_key]) => _key !== key) // 排除同名
      .map(([, value]) => value) // 只保留版本号
      .join(',');
    if (!depsString) return url;
    const separator = url.includes('?') ? '&' : '?';
    return `${url}${separator}deps=${depsString}`;
  };

  const HANDLE_ESM_PACKAGE_TYPE = Object.fromEntries(
    Object.entries(config.ESM_PACKAGE_TYPE).map(([key, url]: any[]) => [
      key,
      addDepsToUrl(key, url),
    ]),
  );
  return {
    DEPS: config.DEPS,
    ESM_PACKAGE_TYPE: HANDLE_ESM_PACKAGE_TYPE,
    UMD_PACKAGE_TYPE: config.UMD_PACKAGE_TYPE,
  };
};
